    ...
    return (
        <svg key={label} width='40' height='64' version="1.1" viewBox="0 0 8.7092 16.514" xmlns="http://www.w3.org/2000/svg">
            <g transform={`translate(${offset})`}>
                <motion.path stroke={color} className={styles.animateTest} {...path.props} strokeWidth="1.6" fill='none' strokeLinecap='round' style={{strokeDashoffset}}/> 
            </g>
        </svg>
    )
}

const Digit = ({digit, strokeDashoffset}) => {
...
const TimerSection = ({digits, label, strokeDashoffset}) => {
            ...
            >
                {
                    (paddedDigit.split('')).map((value, index) => {
                        return <Digit strokeDashoffset={strokeDashoffset} key={`${label}-${index}`} digit={value}/>
                    })
                }
            </div>
            <div
        ...
        <Countdown
            ...
            renderer={({days, formatted}) => {
                ...
                >
                    ...
                    >
                        {
                            (days > 0) && <TimerSection strokeDashoffset={strokeDashoffset} digits={formatted.days} label={days == 1 ? 'Day' : 'Days'}/>
                        }
                        <TimerSection strokeDashoffset={strokeDashoffset} digits={formatted.hours} label={'Hours'}/>
                        <Divider/>
                        <TimerSection strokeDashoffset={strokeDashoffset} digits={formatted.minutes} label={'Minutes'}/>
                        <Divider/>
